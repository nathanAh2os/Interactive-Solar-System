...
const Pluto = ({ DOMAIN_URL, julianEphemerisDate, zoomToObject, searchTerm }) => {
	console.log("Pluto()");
	...
	useEffect(() => {
		console.log("useEffect() pluto orbit");
		let orbitPath = buildOrbitCoordinates(plutoKeplerianValues, julianEphemerisDate, yearLength);
	...
	useEffect(() => {
		console.log("useEffect() pluto position");
		let coords = buildOrbitCoordinates(plutoKeplerianValues, julianEphemerisDate, 1);
		setX(coords[0][0]);
		setY(coords[0][1]);
		setZ(coords[0][2]);
	}, [julianEphemerisDate]);
	...
	if (searchTerm === "pluto") {
		zoomToObject("pluto", zoomLevel, x, y, z);
	}
	...
	return (
		<>
			...
			<Html position={[x,y,z]}>
				<div className="satellite-label" onClick={() => zoomToObject("pluto", zoomLevel, x, y, z)}>Pluto</div>
			</Html>
		</>
	);
};
