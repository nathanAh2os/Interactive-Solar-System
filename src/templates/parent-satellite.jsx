...
const ParentSatellite = ({ data, DOMAIN_URL, julianEphemerisDate, zoomToObject, innerLabels, planetOrbitColor }) => {
	...
	const terms = {
			...
			centerOfRotation: [0,0,0], //Sun coords
	...
	if (data.innerSatellite === true && innerLabels === true) {
		labelJSX = <div className="satellite-label" onClick={() => zoomToObject(satelliteName, "none", zoomLevel, x, y, z)}>{data.name}</div>
	}
	...
	return (
		<>
			...
			<Line points={orbit} color={planetOrbitColor} lineWidth={2} />
 			<Html position={[x, y, z]}>
				{labelJSX}
			</Html>
		</>
